...
Note: The original code does not handle division by zero, so the test case `it('should throw an error when dividing by zero', () => { ... })` will fail unless the original code is modified to throw an error in such cases. However, as per your instructions, I have not changed the original code logic.